...
- **公式**: 遍历按期排序的持仓，跟踪 `peak = max(peak, cum_pnl)`，`dd = peak - cum_pnl`
...
| 标签 | 定义 | 正例 | 反例 |
|------|------|------|------|
| CHASE | 5日涨幅>15% + 价格远超MA20 + 接近20日高点 | 动量追涨 | 正常买入 |
| BOTTOM | 5日跌幅>15% + MA20<MA60 | 下跌趋势中买入 | 回调后正常加仓 |
| BREAKOUT | 入场价>过去20日最高价 + 成交量放大 | 突破买入 | 小幅新高 |
| PYRAMID | 已有持仓时以更高价格加仓 | 加仓时已有未平仓位 | 首次开仓（即使后来盈利） |
| AVERAGE_DOWN | 已有亏损持仓时以更低价格加仓 | 亏损状态下补仓 | 盈利回调后加仓 |
| TURN | 同日既有买入又有卖出 | 日内做T | — |
| SCALP | 持仓 < 3天 | 超短线 | — |
| SWING | 持仓 3-30天 | 波段 | — |
| POSITION | 持仓 > 30天 | 长线持有 | — |
| FOMO | 5日内≥3天上涨 + 入场价≈当日最高价 | 追高情绪交易 | — |
...
| TRAILING_STOP | 持仓>10天 + 盈利<10% | 这不是移动止损行为，只是结果特征 |
...
2. **均线计算**: 本地计算 MA5/MA10/MA20/MA60，基于日线 close
3. **前复权**: 当前未使用复权数据。如未来支持，需在 `market_fetcher.py` 中启用
