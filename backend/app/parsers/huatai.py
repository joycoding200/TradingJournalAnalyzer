...
class HuataiParser(BaseParser):
    ...
    def parse(cls, content: bytes, filename: str) -> list[TradeData]:
        ...
        date_col = _find_col(col_map, ["成交日期", "发生日期", "交易日期", "委托日期", "日期"])
        code_col = _find_col(col_map, ["证券代码", "代码", "合约代码", "合约"])
        price_col = _find_col(col_map, ["成交价格", "成交均价", "价格", "成交价"])
        qty_col = _find_col(col_map, ["成交数量", "数量", "成交量", "手数"])
        side_col = _find_col(col_map, ["买卖方向", "操作", "买卖标志", "方向", "买卖"])
        commission_col = _find_col(col_map, ["手续费", "佣金", "过户费", "印花税"])
